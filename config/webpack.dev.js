...
const configContentScripts = env => {
  ...
  return {
    ...
    plugins: [isProduction ? () => {} : new plugins.ExtensionReloader()],
  };
};

const configPopupAndAssets = env => {
  return {
...
module.exports = env => {
  return [configContentScripts, configPopupAndAssets].map(entry => plugins.webpackMerge([configCommon(env), entry(env)]));
};
